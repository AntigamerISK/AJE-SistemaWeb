...
*************************************************************/
function grogin_local_import_files() {
	return array(
		array(
...
add_filter( 'merlin_import_files', 'grogin_local_import_files' );

/************************************************************
## Setup Wizard After Import
*************************************************************/
function grogin_merlin_after_import_setup() {
	// Assign menus to their locations.
	$main_menu 	  = get_term_by( 'name', 'Menu 1', 'nav_menu' );
	$topleftmenu  = get_term_by( 'name', 'Top Left', 'nav_menu' );
	$toprightmenu = get_term_by( 'name', 'Top Right', 'nav_menu' );
	$canvasbottommenu = get_term_by( 'name', 'Canvas Bottom', 'nav_menu' );
	...
	set_theme_mod(
		'nav_menu_locations', array(
			...
			'canvas-bottom' 	 => $canvasbottommenu->term_id,
    ...
    if ( class_exists( 'woocommerce' ) ) {
        ...
        update_option( 'woocommerce_shop_page_id', '8' );
        update_option( 'woocommerce_cart_page_id', '9' );
        update_option( 'woocommerce_checkout_page_id', '10' );
        update_option( 'woocommerce_myaccount_page_id', '11' );
    }


}
add_action( 'merlin_after_all_import', 'grogin_merlin_after_import_setup' );
