...
function grogin_get_registered_purchase_code() {
	return 'license-' . md5( home_url() . time() );
}

add_action( 'admin_menu', 'grogin_register_theme' );
function grogin_register_theme() {
	add_menu_page( esc_html__( 'KlbTheme', 'grogin' ), esc_html__( 'KlbTheme', 'grogin' ), 'manage_options', 'klbtheme_options', 'grogin_register_theme_options', 'dashicons-welcome-widgets-menus', 2);
	add_submenu_page( 'klbtheme_options', esc_html__('Register Theme', 'grogin'), esc_html__('Register Theme', 'grogin'), 'manage_options', 'klbtheme_options', 'grogin_register_theme_options' );
	add_submenu_page( 'klbtheme_options', esc_html__('Theme Options', 'grogin'), esc_html__('Theme Options', 'grogin'), 'manage_options', 'customize.php?return=admin.php?page=klbtheme_options' );
}

function grogin_register_theme_options() {
	...
	<?php if(!empty(grogin_get_registered_purchase_code())){ ?>
		<div class="klb-help-container registered-container">
			...
			<div class="registered-right"> 
				<h2><?php esc_html_e('The theme registered','grogin'); ?></h2>
				<p><?php esc_html_e('Envato allows 1 license for 1 project located on 1 domain. It means that 1 purchase key can be used only for 1 site. Each additional site will require an additional purchase key.','grogin'); ?></p>
			</div>
		</div>
	<?php } ?>
	...
	<div class="klb-help-container">
		<div class="klb-help-item">
			<a href="https://klbtheme.com/doc/grogin" target="_blank">
				...
				<h2><?php esc_html_e('Documentation', 'grogin'); ?></h2>
			</a>
		</div>
		...
		<div class="klb-help-item">
			<a href="https://themeforest.net/user/klbtheme#contact" target="_blank">
				...
				<h2><?php esc_html_e('Reset License', 'grogin'); ?></h2>
			</a>
		</div>
		...
		<div class="klb-help-item">
			<a href="https://klbtheme.ticksy.com/" target="_blank">
				...
				<h2><?php esc_html_e('Support', 'grogin'); ?></h2>
			</a>
		</div>
		...
		<div class="klb-help-item">
			<a href="https://klbtheme.com/hire-us/" target="_blank">
				...
				<h2><?php esc_html_e('Hire Us', 'grogin'); ?></h2>
			</a>
		</div>
	</div>
	...
	if(empty(grogin_get_registered_purchase_code())){
		echo '<form action="" method="post" id="purchase_code_form">';
		echo '<h1>'.esc_html__('Register Theme','grogin').'</h1>';
		echo '<p style="max-width: 500px;">'.esc_html__('You\'re almost done. Just one more step. In order to gain full access to all demos, 
		premium plugins and support, please register your theme\'s purchase code.','grogin').'</p>';
		echo '<h2>'.esc_html__('Your Envato Purchase Code','grogin').'</h2>';
		
		echo '<p>';
		echo '<input class="regular-text code" type="text" name="purchase_code" id="purchase_code" value=""> ';
		echo '<a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank">'.esc_html__('Where to find the code?','grogin').'</a>';
		echo '</p>';
		
		echo '<p>';
		echo '<label>';
		echo '<input type="checkbox" id="klb-accept-license-terms">'.esc_html__('I confirm that, according to the Envato License Terms, I am licensed to use the purchase code for a single project. Using it on multiple installations is a copyright violation.','grogin').'</label>';
		echo '<a href="https://themeforest.net/licenses/terms/regular" target="_blank">'.esc_html__('License details.','grogin').'</a>';
		echo '</p>';
		
		echo '<p class="klb-actions">';
		echo '<button class="button button-primary button-hero" id="klb-register-theme" disabled="disabled">'.esc_html__('Register Theme','grogin').'</button>';
		echo '</p>';
		...
		<div class="c-inner klbtheme-status">
			<h2><?php esc_html_e( 'WordPress', 'grogin' ); ?></h2>
			<div class="klb-table klb-odd">
				<div class="klb-table-row">
					<div>
						<?php esc_html_e( 'Theme Name', 'grogin' ); ?>:
					</div>
					<div>
				...
				<div class="klb-table-row">
					<div>
						<?php esc_html_e( 'Theme Version', 'grogin' ); ?>:
					</div>
					<div>
				...
				<div class="klb-table-row">
					<div>
						<?php esc_html_e( 'WP Version', 'grogin' ); ?>:
					</div>
					<div>
				...
				<div class="klb-table-row">
					<div>
						<?php esc_html_e( 'WP Multisite', 'grogin' ); ?>:
					</div>
					<div>
						<?php echo is_multisite() ? esc_html__( 'Yes', 'grogin' ) : esc_html__( 'No', 'grogin' ); ?>
					</div>
				</div>

				<div class="klb-table-row">
					<div>
						<?php esc_html_e( 'WP Debug Mode', 'grogin' ); ?>:
					</div>
					<div>
						<?php echo defined( 'WP_DEBUG' ) && WP_DEBUG ? esc_html__( 'Enabled', 'grogin' ) : esc_html__( 'Disabled', 'grogin' ); ?>
					</div>
				</div>
			</div>
			<h2><?php esc_html_e( 'Server', 'grogin' ); ?></h2>
			<div class="klb-table klb-odd">

				<div class="klb-table-row">
					<div>
						<?php esc_html_e( 'PHP Version', 'grogin' ); ?>:
					</div>
					<div>
						<?php if ( version_compare( PHP_VERSION, '7.2', '<' ) ) : ?>
							<div class="klb-status-error">
								...
								<span>
									<?php esc_html_e( 'Minimum required PHP version 7.2', 'grogin' ); ?>
								</span>
							</div>
						<?php else : ?>
							<?php echo esc_html( PHP_VERSION ); ?>
				...
				<?php if ( function_exists( 'ini_get' ) ) : ?>
					<div class="klb-table-row">
						<div>
							...
							<?php esc_html_e( 'PHP Post Max Size', 'grogin' ); ?>:
						</div>
						...
						<div>
							<?php if ( wp_convert_hr_to_bytes( $post_max_size ) < 64000000 ) : ?>
								<div class="klb-status-error">
									...
									<span>
										<?php esc_html_e( 'Minimum required value 64M.', 'grogin' ); ?>
									</span>
								</div>
							<?php else : ?>
								<?php echo esc_html( $post_max_size ); ?>
					...
					<div class="klb-table-row">
						<div>
							...
							<?php esc_html_e( 'PHP Time Limit', 'grogin' ); ?>:
						</div>
						...
						<div>
							<?php if ( $max_execution_time < 180 ) : ?>
								<div class="klb-status-error">
									...
									<span>
										<?php esc_html_e( 'Minimum required value 180.', 'grogin' ); ?>
									</span>
								</div>
							<?php else : ?>
								<?php echo esc_html( $max_execution_time ); ?>
					...
					<div class="klb-table-row">
						<div>
							...
							<?php esc_html_e( 'PHP Max Input Vars', 'grogin' ); ?>:
						</div>
						...
						<div>
							<?php if ( $max_input_vars < 10000 ) : ?>
								<div class="klb-status-error">
									...
									<span>
										<?php esc_html_e( 'Minimum required value 10000.', 'grogin' ); ?>
									</span>
								</div>
							<?php else : ?>
								<?php echo esc_html( $max_input_vars ); ?>
					...
					<div class="klb-table-row">
						<div>
							...
							<?php esc_html_e( 'PHP Memory Limit', 'grogin' ); ?>:
						</div>
						...
						<div>
							<?php if ( wp_convert_hr_to_bytes( $memory_limit ) < 128000000 ) : ?>
								<div class="klb-status-error">
									...
									<span>
										<?php esc_html_e( 'Minimum required value 128M.', 'grogin' ); ?>
									</span>
								</div>
							<?php else : ?>
								<?php echo esc_html( $memory_limit ); ?>
					...
					<div class="klb-table-row">
						<div>
							...
							<?php esc_html_e( 'PHP Upload Max Size', 'grogin' ); ?>:
						</div>
						<div>
					...
					<div class="klb-table-row">
						<div>
							<?php esc_html_e( 'PHP Function "file_get_content"', 'grogin' ); ?>:
						</div>
						<div>
							<?php if ( ! ini_get( 'allow_url_fopen' ) || 'Off' === ini_get( 'allow_url_fopen' ) ) : ?>
								<div class="klb-status-error">
									<?php esc_html_e( 'Off', 'grogin' ); ?>
								</div>
							<?php else : ?>
								<?php esc_html_e( 'On', 'grogin' ); ?>
							<?php endif; ?>
						</div>
					</div>
				<?php endif; ?>
				...
				<div class="klb-table-row">
					<div>
						<?php esc_html_e( 'DOMDocument', 'grogin' ); ?>:
					</div>
					<div>
						<?php if ( ! class_exists( 'DOMDocument' ) ) : ?>
							<div class="klb-status-error">
								<?php esc_html_e( 'No', 'grogin' ); ?>
							</div>
						<?php else : ?>
							<?php esc_html_e( 'Yes', 'grogin' ); ?>
						<?php endif; ?>
					</div>
				</div>
				...
				<div class="klb-table-row">
					<div>
						<?php esc_html_e( 'Active Plugins', 'grogin' ); ?>:
					</div>
					<div>
						<?php if ( is_multisite() ) : ?>
	...
	if(isset($_POST['purchase_code']) && wp_verify_nonce( $_POST['register_theme_nonce_field'], 'register_theme_my_action' )) {
		...
		echo '<div class="data-response success">'.esc_html__('The theme registered succesfully','grogin').'</div>';
		update_option( 'envato_purchase_code_50297090', $purchase_code );
	}
}


function grogin_is_theme_registered() {
	return true;
}
...
$grogin_tgmpa_prefix = ( defined( 'WP_NETWORK_ADMIN' ) && WP_NETWORK_ADMIN ) ? 'network_admin_' : '';
add_filter( 'tgmpa_' . $grogin_tgmpa_prefix . 'plugin_action_links', 'grogin_tgmpa_filter_action_links', 10, 4 );
function grogin_tgmpa_filter_action_links( $action_links, $item_slug, $item, $view_context ) {
	$source = ! empty( $item['source'] ) ? $item['source'] : '';

	// Prevent installing theme's premium plugins.
	if ( 'External Source' === $source && ! grogin_is_theme_registered() ) {
		$action_links = array(
			'grogin_registration_required' => sprintf( __( '<a style="color: #ff0000;" href="%s">Register theme to unblock it</a>', 'grogin' ), esc_url( admin_url( 'themes.php?page=register-theme' ) ) ),
		);
	}
 ...
 * Admin Notice
 */
add_action('admin_notices', 'grogin_notice_for_activation');
function grogin_notice_for_activation() {

	if(empty(grogin_get_registered_purchase_code())){

		echo '<div class="notice notice-warning">
			<p>' . sprintf(
			esc_html__( 'Enter your Envato Purchase Code to receive Theme and plugin updates %s', 'grogin' ),
			'<a href="' . admin_url('admin.php?page=klbtheme_options') . '">' . esc_html__( 'Enter Purchase Code', 'grogin' ) . '</a>') . '</p>
		</div>';
	}
}