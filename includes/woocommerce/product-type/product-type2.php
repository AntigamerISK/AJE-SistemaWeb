...
/*----------------------------
  Product Type 2
 ----------------------------*/
function grogin_product_type2($stockprogressbar = '', $stockstatus = '', $shippingclass = '', $countdown = '', $product_type = '', $shortdesc = '', $product_sku = '', $productattributes = ''){
	global $product;
	global $post;
	global $woocommerce;
	
	if($product_type != 'type2') {
		return;
	}
	...
	$gallery = get_theme_mod('grogin_product_box_gallery') == 1 ? 'product-thumbnail' : '';
		...
		if($shortdesc == '1' && $short_desc){
		$output .= '<div class="product-excerpt entry-excerpt">';
		$output .= '<p class="text-12 text-gray-600">'.grogin_limit_words($short_desc, 11).'</p>';
...
add_action('klb_product_box','grogin_product_type2', 10, 7); 